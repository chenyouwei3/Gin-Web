...
func AddComment(commentForm model.AddCommentForm) utils.Response {
	if err := global.CommentTableMaster.Transaction(func(tx *gorm.DB) error {
...
func AddMoment(addMomentForm model.AddMomentForm) utils.Response {
	if err := global.MomentTableMaster.Transaction(func(tx *gorm.DB) error {
		moment := model.Moment{
			Id:         global.MomentSnowFlake.Generate().Int64(),
			CreateTime: time.Now(),
